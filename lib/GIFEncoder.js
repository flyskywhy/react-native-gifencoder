...
class GIFEncoder {
	...
	finish() {
		...
		for (const analysisResultOrFunction of this.frames) {
			...
			if (firstFrame) {
				gifWriter.writeColorTable(colorTable); // global color table
				if (this.repeat >= 0) {
					// use NS app extension to indicate reps
					gifWriter.bytes.writeByte(0x21); // extension introducer
					gifWriter.bytes.writeByte(0xff); // app extension label
					gifWriter.bytes.writeByte(11); // block size
					gifWriter.bytes.writeUTFBytes('NETSCAPE' + '2.0'); // app id + auth code
					gifWriter.bytes.writeByte(3); // sub-block size
					gifWriter.bytes.writeByte(1); // loop sub-block id
					gifWriter.writeShort(this.repeat); // loop count (extra iterations, 0=repeat forever)
					gifWriter.bytes.writeByte(0); // block terminator
				}
			}
