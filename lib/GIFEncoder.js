...
class GIFEncoder {
	...
	encode() {
		...
		for (const image of this.images) {
			const rgbaPixels = image[0];
			const left = (image[1] !== undefined) ? image[1] : 0;
			const top = (image[2] !== undefined) ? image[2] : 0;
			const width = (image[3] !== undefined) ? image[3] : this.width;
			const height = (image[4] !== undefined) ? image[4] : this.height;
			const disposalMethod = (image[5] !== undefined) ? image[5] : this.dispose;
			const delayTime = (image[6] !== undefined) ? image[6] : this.delay;
			const transparentColor = (image[7] !== undefined) ? image[7] : this.transparent;

			// Remove alpha channel from pixel array
			const rgbPixels = rgbaPixelsToRgbPixels(rgbaPixels);

			//
			const analysisResult = analyzePixels(rgbPixels, this.sample, usedEntry);
			const localColorTable = analysisResult[0];
			const indexedPixels = analysisResult[1];


			const tableBasedImageData = new ByteArray();
			var lzwEncoder = new LZWEncoder(this.width, this.height, indexedPixels, 8);
			lzwEncoder.encode(tableBasedImageData);

			// get closest match to transparent color if specified
			let transparentColorIndex;
			if (transparentColor !== undefined) {
				transparentColorIndex = findClosestColorIndex(localColorTable, transparentColor, usedEntry);
			}
	...
	constructor() {
		...
		this.height;
		this.transparent = undefined; // transparent color if given
		this.repeat = -1; // no repeat
		this.delay = 0; // frame delay (hundredths)
		this.dispose = undefined; // disposal code (-1 = use default)
