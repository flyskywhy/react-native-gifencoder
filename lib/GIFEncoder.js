...
class GIFEncoder {
	...
	addFrame(frame) {
		...
		// Check if the user started
		if (!this.started || this.out === null) {
			throw new Error('start() must be used before using addFrame().');
		}
		...
		try {
			...
			const analysisResult = this.analyzePixels(rgbPixels, this.sample); // build color table & map pixels
			const colorTab = analysisResult[0];
			const indexedPixels = analysisResult[1];
			const transIndex = analysisResult[2];
			const colorDepth = 8;
			const palSize = 7;
			...
			this.writeImageDesc(palSize); // image descriptor
	...
	analyzePixels(pixels, sample) {
		...
		return [colorTab, indexedPixels, transIndex];
	}
